...
const identityRules: Rule[] = [
  ...
  { type: "history_heritage", pattern: /궁\b|성곽|산성|유적|사적|고분|서원|향교|사찰|암자|성당|생가|기념비|노래비/u, label: "역사·유산 명칭" },
...
export function assessPlaceExperienceType(place: ExperienceTypeInput): PlaceExperienceTypeAssessment {
  const identity = identityRules.find((rule) => rule.pattern.test(normalize(place.name)));
  ...
  if (contentType) return { type: contentType, confidence: 55, evidence: ["한국관광공사 관광 유형"] };
  if (place.moodTags.includes("walk") || place.moodTags.includes("open")) {
    return { type: "forest_garden", confidence: 35, evidence: ["산책·개방감 태그"] };
  }
...
function fromContentType(contentType?: string | null): PlaceExperienceType | null {
  ...
  if (contentType === "14") return "museum_story";
  return null;
}
