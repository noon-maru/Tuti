...
- 외부 심사에는 메일함에 의존하지 않는 고정 심사용 이메일과 비밀번호를
  제공한다. `AUTH_APP_REVIEW_EMAIL`과 `AUTH_APP_REVIEW_PASSWORD`를 모두
  설정한 경우 해당 이메일만 비밀번호 입력 흐름으로 전환한다. 해당 이메일도
  일반 사용자와 동일하게 요청 제한과 10분 유효한 해시 챌린지를 거친다.
  실제 비밀번호는 서버 환경변수와 심사 정보에만 저장한다. 기존 숫자 6자리
  `AUTH_APP_REVIEW_CODE`는 하위 호환용이며 비밀번호 설정이 우선한다.

- 이메일 로그인 요청·검증 결과는 시스템 감사 로그에 성공 여부,
  SMTP·심사용 비밀번호·기존 고정 코드 경로, 오류 코드와 처리 시간만
  기록한다. 이메일 주소, 인증코드, 비밀번호, IP 주소는 감사 로그에
  남기지 않는다.
