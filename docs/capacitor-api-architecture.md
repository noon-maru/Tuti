...
- App Review에는 외부 메일함에 의존하지 않는 고정 심사용 이메일과
  숫자 6자리 코드를 제공한다. `AUTH_APP_REVIEW_EMAIL`과
  `AUTH_APP_REVIEW_CODE`를 모두 설정한 경우에만 활성화된다. 둘 중 하나만
  설정된 잘못된 배포에서는 인증 요청을 차단한다. 해당 이메일도
  일반 사용자와 동일하게 코드 요청 후 10분 유효한 해시 챌린지를 거친다.
  실제 값은 저장소에 커밋하지 않는다.

- 이메일 인증코드 요청·검증 결과는 시스템 감사 로그에 성공 여부,
  SMTP 또는 심사용 고정 코드 경로, 오류 코드와 처리 시간만 기록한다.
  이메일 주소, 인증코드, IP 주소는 감사 로그에 남기지 않는다.
