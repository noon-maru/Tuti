...
for operation in tuti-prod-deploy tuti-dev-refresh tuti-dev-restart tuti-docker-status tuti-tourism-sync tuti-tourism-bootstrap; do
...
printf '%s\n' \
  "${admin_user} ALL=(root) NOPASSWD: /usr/local/sbin/tuti-prod-deploy, /usr/local/sbin/tuti-dev-refresh, /usr/local/sbin/tuti-dev-restart, /usr/local/sbin/tuti-docker-status, /usr/local/sbin/tuti-tourism-sync, /usr/local/sbin/tuti-tourism-bootstrap" \
  > "$sudoers_file"
